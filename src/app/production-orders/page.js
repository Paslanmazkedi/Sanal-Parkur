...
export default function ProductionOrdersPage() {
  ...
  const loadOrders = async () => {
    const { data } = await supabase.from('production_orders').select('*').order('id', { ascending: false });
  ...
  const handleCreateOrder = async (e) => {
    e.preventDefault();
    setLoading(true);
    const { error } = await supabase.from('production_orders').insert([{ code: woCode, total_qty: woQty, status: 1 }]);
    if (!error) {
