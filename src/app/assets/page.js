  ...
  return (
    <div className="w-full min-w-0 max-w-full">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-bold sm:text-2xl">İstasyonlar</h1>
        <button
          onClick={() => setShowModal(true)}
          className="w-full shrink-0 rounded bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 sm:w-auto"
        >
          Yeni İstasyon Ekle +
        </button>
      </div>
      ...
      {/* Stations Table */}
      <div className="w-full min-w-0 overflow-x-auto rounded-xl border border-slate-800 bg-slate-900/60">
        <table className="min-w-[36rem] w-full bg-slate-900/40">
    ...
    <span className="text-rose-400 bg-rose-500/10 px-2 py-1 rounded text-xs">Pasif</span>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-4 shadow-lg sm:p-6">
